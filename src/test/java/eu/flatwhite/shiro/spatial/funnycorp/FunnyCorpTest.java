...
public class FunnyCorpTest {
	...
	//
	// The vending machine (that runs with embedded Java and uses Shiro as
	// security framework) shall
	// obey the following rules:
	//
	// 1 - all the girls may have a coffee
	// 2 - all the boys may have a coke
	// 3 - all the "merited" employees (having badgeNo less or equal 10) may
	// have a beer
	//
	// Note: this is an example of "misuse" of spatiality, since we do not
	// have a real "space" defined here
	// in it's Euclidean way, but still, by cheating with relation providers
	// we can do even this.
	...
	// since we "cheat" with relation provider, it is completely indifferent
	// who we provide in permission as spatial
	// so we use a "template" person, that actually provides "coordinates"
	// to relate to. So, we use a special person
	// that is actually a "template person", and just serves as source for
	// space to get some coordinates and
	...
	// the important thing is the "space" in which we place that template
	// person
	Permission coffePermission = new WildcardPermission("coffee");

	SpatialPermission coffeSpatialPermission = new SpatialPermission(
		new EnumSpatial(genderSpace, Gender.FEMALE),
	...
	SpatialPermission cokeSpatialPermission = new SpatialPermission(
		new EnumSpatial(genderSpace, Gender.MALE),
	...
	// Rule 3 - all the "merited" employees (having badgeNo less or equal
	// 10) may have a beer
	// Merit space is non-finite space, and the "merit threshold" is defined
	// as "10 or less", so we
	...
	SpatialPermission beerSpatialPermission = new SpatialPermission(
		new Point(meritSpace, 10), new SphereRelationProvider(),
		permissions);
	...
	SpatialResolver personResolver = new SpatialResolver() {
		...
		people.add(new Person(personSpace, "Kristine", Gender.FEMALE,
			12));
		...
		for (Person p : people) {
		    if (spatialString.equalsIgnoreCase(p.getName()))
			return p;
		}
		throw new InvalidPermissionStringException("no such person",
			spatialString);
	    }
	};
	...
	SpaceResolver spaceResolver = MapSpaceResolver.Builder.newSpaceMap()
		.addSpace("person", personSpace)
		.addSpace("gender", genderSpace).addSpace("merit", meritSpace)
		.build();
	SpatialResolver spatialResolvers = MapSpatialResolver.Builder.newMap()
		.add(personSpace, personResolver)
		.add(genderSpace, genderResolver)
		.add(meritSpace, meritResolver).build();
	SpaceRelationProvider spaceRelationProviders = MapSpaceRelationProvider.Builder
		.newMap().add(personSpace, new PointRelationProvider())
		.add(genderSpace, new PointRelationProvider())
		.add(meritSpace, new SphereRelationProvider()).build();
	SpatialPermissionResolver resolver = new SpatialPermissionResolver(
		spaceResolver, spatialResolvers, spaceRelationProviders);
	...
	// the important thing is the "space" in which we place that template
	// person
	Permission coffeSpatialPermission = resolver
		.resolvePermission("gender:female:coffee");
	Assert.assertEquals(false, coffeSpatialPermission.implies(resolver
		.resolvePermission("person:Jason:coffee")));
	Assert.assertEquals(false, coffeSpatialPermission.implies(resolver
		.resolvePermission("person:Thomas:coffee")));
	Assert.assertEquals(true, coffeSpatialPermission.implies(resolver
		.resolvePermission("person:Kristine:coffee")));
	Assert.assertEquals(false, coffeSpatialPermission.implies(resolver
		.resolvePermission("person:Damian:coffee")));
	Assert.assertEquals(false, coffeSpatialPermission.implies(resolver
		.resolvePermission("person:Toby:coffee")));
	Assert.assertEquals(true, coffeSpatialPermission.implies(resolver
		.resolvePermission("person:Linda:coffee")));

	// Rule 2 - all the boys may have a coke
	// gender space is finite space, so SamePointRelation provider does it
	Permission cokeSpatialPermission = resolver
		.resolvePermission("gender:male:coke");
	Assert.assertEquals(true, cokeSpatialPermission.implies(resolver
		.resolvePermission("person:Jason:coke")));
	Assert.assertEquals(true, cokeSpatialPermission.implies(resolver
		.resolvePermission("person:Thomas:coke")));
	Assert.assertEquals(false, cokeSpatialPermission.implies(resolver
		.resolvePermission("person:Kristine:coke")));
	Assert.assertEquals(true, cokeSpatialPermission.implies(resolver
		.resolvePermission("person:Damian:coke")));
	Assert.assertEquals(true, cokeSpatialPermission.implies(resolver
		.resolvePermission("person:Toby:coke")));
	Assert.assertEquals(false, cokeSpatialPermission.implies(resolver
		.resolvePermission("person:Linda:coke")));
	...
	// as "10 or less", so we
	// need to do it like this:
	Permission beerSpatialPermission = resolver
		.resolvePermission("merit:10:beer:beer");

	Assert.assertEquals(true, beerSpatialPermission.implies(resolver
		.resolvePermission("person:Jason:beer")));
	Assert.assertEquals(true, beerSpatialPermission.implies(resolver
		.resolvePermission("person:Thomas:beer")));
	Assert.assertEquals(false, beerSpatialPermission.implies(resolver
		.resolvePermission("person:Kristine:beer")));
	Assert.assertEquals(true, beerSpatialPermission.implies(resolver
		.resolvePermission("person:Damian:beer")));
	Assert.assertEquals(false, beerSpatialPermission.implies(resolver
		.resolvePermission("person:Toby:beer")));
	Assert.assertEquals(false, beerSpatialPermission.implies(resolver
		.resolvePermission("person:Linda:beer")));
    }

    protected void check(boolean impliesExpected, SpatialPermission sp,
	    Person person, Permission vendingMachineContentPermission) {
	Assert.assertEquals(impliesExpected, sp.implies(new SpatialPermission(
		person, new PointRelationProvider(),
		vendingMachineContentPermission)));
    }
}
