...
/**
 * Resolves {@code Point} instances from a {@code String}. This implementation
 * parses a comma-separated list of {@code Double}.
 * 
...
public class PointResolver implements SpatialResolver {
  ...
  @Override
	...
	if (es.getDimensions() != points.length)
	    throw new InvalidPermissionStringException(
		    "invalid number of points", spatialString);
	...
	for (int i = 0; i < points.length; i++) {
		...
		throw new InvalidPermissionStringException(
			"cannot parse point", spatialString);
	    }
	}
